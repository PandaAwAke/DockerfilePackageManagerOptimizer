...
class PMHandler(object):
    ...
    def __init__(self, global_status: GlobalStatus):
        ...
        self._load_pm_settings()

    def is_package_manager_executable(self, executable: str) -> bool:
        for pm_name, pm_setting in self.pm_settings.items():
            if executable in pm_setting.executables:
                return True
        return False

    def _load_pm_settings(self):
        try:
            f = open(file='src/resources/PMSettings.yaml', mode='r', encoding='utf-8')
            pm_settings = yaml.safe_load(f)
        except Exception as e:  # Including: IOError, yaml.YAMLError
            logging.error(e)
            exit(-1)
        pm_settings: dict = pm_settings['packageManagers']
        for pm_name in pm_settings.keys():
            pm_setting_dict: dict = pm_settings[pm_name]
            pm_setting = PMSetting(
                executables=pm_setting_dict.get('executables') or [pm_name],
                commands_regex_run=pm_setting_dict.get('commands-regex-run') or [],
                default_cache_dirs=pm_setting_dict.get('default-cache-dirs') or [],
                commands_regex_modify_cache_dir=pm_setting_dict.get('commands-regex-modify-cache-dir') or [],
                additional_pre_commands=pm_setting_dict.get('additional-pre-commands') or []
            )
            ...
            if len(pm_setting.default_cache_dirs) == 0:
                ...
                exit(-1)
            self.pm_settings[pm_name] = pm_setting
        f.close()
    ...
    def handle(self, commands: list, instruction_index: int):
        ...
        for command in commands:
            ...
            if pm_name not in self.pm_statuses.keys():
                self.pm_statuses[pm_name] = PMStatus(
                    cache_dirs=[self._replace_home_char(cache_dir)
                                for cache_dir in self.pm_settings[pm_name].default_cache_dirs]
                )
            pm_status: PMStatus = self.pm_statuses[pm_name]
            command_str = ' '.join(command[1:])

            # Case for modifying the cache dir
            for command_regex_modify_cache_dir in self.pm_settings[pm_name].commands_regex_modify_cache_dir:
            ...
            for command_regex_run in self.pm_settings[pm_name].commands_regex_run:
        ...
        if need_to_optimize:
            ...
            if len(self.pm_settings[pm_name].additional_pre_commands) > 0 and \
                    self.pm_statuses[pm_name].pre_commands_added == False:
                self.optimization_strategies.append(InsertBeforeStrategy(
                    instruction_index=instruction_index,
                    commands_insert=self.pm_settings[pm_name].additional_pre_commands
                ))
