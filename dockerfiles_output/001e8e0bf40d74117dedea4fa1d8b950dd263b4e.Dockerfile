...
RUN --mount=type=cache,target=/root/.cache/go-build go get github.com/tcnksm/ghr && \
...
RUN --mount=type=cache,target=/var/lib/apt --mount=type=cache,target=/var/cache/apt apt-get update -y && \
